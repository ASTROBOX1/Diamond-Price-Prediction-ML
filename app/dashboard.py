...
# --- DATA LOADING FUNCTIONS (Called outside render_dashboard) ---
@st.cache_resource
def load_predictor():
    """Load the diamond price prediction model"""
    return DiamondPricePredictor()

@st.cache_data
def load_market_data():
    """Load market data for analytics"""
    try:
...
def create_scatter_chart(df):
    """Create scatter plot for price vs carat"""
...
def create_box_chart(df):
    """Create box plot for valuation distribution"""
    fig_box = px.box(
        df, 
        x="cut", 
        y="price", 
...
def render_dashboard():
    """
    Render the complete diamond price prediction dashboard.
    This function is called from streamlit_app.py after st.set_page_config().
    """
    ...
    with right_col:
        ...
        if df is not None:
            ...
            st.plotly_chart(create_box_chart(df), use_container_width=True)
        else:
